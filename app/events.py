...
import qrcode
from flask import Blueprint, flash, jsonify, redirect, render_template, request, session, send_file, url_for
from flask_wtf.csrf import generate_csrf

from app.auth import admin_required, login_required
from app.database import (
    create_event,
    get_all_events_with_stats,
    get_event_by_id,
...
def react_spa():
    spa_index = Path(__file__).resolve().parent.parent / "static" / "spa-react-build" / "index.html"
    if spa_index.exists():
        return send_file(spa_index)
    fallback_index = Path(__file__).resolve().parent.parent / "static" / "spa-react" / "index.html"
    if fallback_index.exists():
        return send_file(fallback_index)
...
def buy_ticket(event_id):
    ...
    if result == "expired":
        flash("Это событие уже прошло", "error")
        return redirect(url_for("events.event_page", event_id=event_id))
    if result != "success":
    ...
    try:
        ...
        print(f"Ошибка генерации PDF: {exc}")
...
def download_ticket(ticket_id):
    ...
    return send_file(pdf_path, as_attachment=True, download_name=f"ticket_{ticket_id[:8]}.pdf")
...
def api_get_events():
    ...
    if date_filter in {"this_week", "next_month"}:
        now = datetime.now()
        cutoff = now + (timedelta(days=7) if date_filter == "this_week" else timedelta(days=31))
        events_list = [
            event
            for event in events_list
            if (dt := parse_event_datetime(event.get("date"))) and now <= dt <= cutoff
        ]
    ...
    for event in events_list:
        ...
        event["cover_url"] = url_for("static", filename=f"event_covers/{cover}") if cover else None
...
def api_create_event():
    from app.admin import _save_cover_image, _save_ticket_template
    ...
    if template_file and template_file.filename:
        ...
        if ticket_template is None:
            return jsonify({"ok": False, "message": "Некорректный PDF-шаблон"}), 400

    cover_image = None
    if cover_file and cover_file.filename:
        cover_image = _save_cover_image(cover_file)
        if cover_image is None:
            return jsonify({"ok": False, "message": "Некорректное изображение обложки"}), 400
